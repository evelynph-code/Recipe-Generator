...
export const generateRecipe = async ({ingredients, dietaryRestrictions = [], cuisineType = 'any', servings = 4, cookingTime = 'medium'}) => {
    ...
    Please provide a complete recipe in the folowing JSON format (return ONLY valid JSON, no markdown):
    {
    "name": "Recipe name",
    ...
    "Step 1 description",
    "Step 2 description"],
    "dietaryTags": ["vegetarian", "gluten_free", etc.],
    "nutrition": {
    "calories": number,
...
export const generatePantrySuggestions = async (PantryItems, expiringItems = []) => {
    ...
    const prompt = `Based on these available ingredents: ${ingredients}${expiringText}
...
Each suggestion should be a bried, appetizing description (1-2 sentences).`;

...
class MealPlan {
    ...
    static async findByDateRange(userId, startDate, endDate) {
        const imageColumn = await Recipe.getImageColumn();
        const result = await db.query(
            `SELECT mp.id, mp.user_id, mp.recipe_id, mp.meal_date::text as meal_date,
            mp.meal_type, mp.created_at, mp.updated_at,
            r.name as recipe_name, r.${imageColumn} AS image_url, r.prep_time, r.cook_time
        FROM meal_plans mp
    ...
    /**
     * Get upcomming meals (next 7 days)
     */
    static async getUpcoming(userId, limit = 5) {
        const imageColumn = await Recipe.getImageColumn();
        const result = await db.query(
            `SELECT mp.*, r.name as recipe_name, r.${imageColumn} AS image_url
        FROM meal_plans mp
        JOIN recipes r ON mp.recipe_id = r.id
        WHERE mp.user_id = $1
